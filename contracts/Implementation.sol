...
contract Implementation{
    ...
    function sit(string memory roomName, string memory kind, bool choice, uint128 amount) external{
        ...
        console.log("two");
        // require(
        //     amount <= IERC20(token).balanceOf(msg.sender) && amount <= IERC20(token).allowance(msg.sender, address(this)),
        //     "insufficient balcnce or allowance"
        // );//下注前检查用户余额和合约的allowance
        // if(amount > chips[msg.sender]){//如果筹码不够
        //     IERC20(token).safeTransferFrom(msg.sender, address(this), amount);//向合约转入新筹码chips
        // }
    ...
    //离场时取回chip，chip包含两部分，投入的部分，游戏过程中赢或者输的数量
